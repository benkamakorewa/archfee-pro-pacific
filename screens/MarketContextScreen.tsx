...
import { ProjectData, ConstructionStandard, LandType } from '../types';
...
const MarketContextScreen: React.FC<MarketContextScreenProps> = ({ data, onUpdate, onNext, onBack }) => {
  ...
  const standards = Object.values(ConstructionStandard);
              ...
              {standards.map(s => (
                <button
                  ...
                  className={`w-full flex items-center justify-between p-4 rounded-xl border-2 transition-all ${
                    data.standard === s ? 'border-[#13a4ec] bg-[#13a4ec]/5 text-[#13a4ec]' : 'border-gray-50 bg-gray-50 text-gray-500'
                  }`}
                >
                ...
                {landTypes.map(t => (
                  <button
                    ...
                    className={`p-3 rounded-lg border-2 text-[11px] font-black uppercase tracking-widest transition-all ${
                      data.landType === t ? 'border-[#13a4ec] bg-[#13a4ec]/5 text-[#13a4ec]' : 'border-gray-50 bg-gray-50 text-gray-400'
                    }`}
                  >
                    {t}
                  </button>
